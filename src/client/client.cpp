      ...
      status(NotActive), alive_count(1), swap_pile(0)
{
    ...
    // Disha
    m_interactions[S_COMMAND_REBOUND] = &Client::askForRebound;
    m_interactions[S_COMMAND_COVER] = &Client::askForCover;
    m_interactions[S_COMMAND_SUDDENSTRIKE] = &Client::askForSuddenStrike;
    m_interactions[S_COMMAND_ROB] = &Client::askForRob;
...
// Disha
void Client::askForCover(const Json::Value &arg){
    if (!arg.isArray() || arg.size() != 3 || !arg[0].isString()
            || !(arg[1].isNull() ||arg[1].isString())
            || !arg[2].isString()) return;

    QStringList prompt;
    prompt << "@cover" << toQString(arg[2]) << toQString(arg[0]);
...
void Client::askForRebound(const Json::Value &arg){
    if (!arg.isArray() || arg.size() != 3 || !arg[0].isString()
            || !(arg[1].isNull() ||arg[1].isString())
            || !arg[2].isString()) return;

    QStringList prompt;
    prompt << "@rebound" << toQString(arg[1]) ;
...
void Client::askForSuddenStrike(const Json::Value &arg){
    if (!arg.isArray() || arg.size() != 3 || !arg[0].isString()
            || !(arg[1].isNull() ||arg[1].isString())
            || !arg[2].isString()) return;

    QStringList prompt;
    prompt << "@sudden_strike" << toQString(arg[2]) ;
...
void Client::askForRob(const Json::Value &arg){
    if (!arg.isArray() || arg.size() != 3 || !arg[0].isString()
            || !(arg[1].isNull() ||arg[1].isString())
            || !arg[2].isString()) return;

    QStringList prompt;
    prompt << "@rob" << toQString(arg[2]) ;
